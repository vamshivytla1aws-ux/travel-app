...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await query<{ id: number }>(`SELECT id FROM drivers WHERE id = $1`, [id]);
  ...
  const duplicate = await query<{ id: number }>(
    ...
    [payload.core.phone, payload.core.licenseNumber, id],
  );
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (pgError?.code === "23503") {
      return NextResponse.json({ error: "Driver has dependency references" }, { status: 409 });
    }
    throw error;
  }
}
